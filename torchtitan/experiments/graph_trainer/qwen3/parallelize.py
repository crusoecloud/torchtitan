...
def annotate_qwen3(model: GraphTrainerQwen3Model) -> None:
    ...
    if model.config.layer.moe_enabled:
...
def parallelize_qwen3(
    ...
    *,
    ...
    # Store the mixed precision param dtype so that init_weights can cast
    # the cos/sin RoPE cache (freqs_cis) to match FSDP2's cast_forward_inputs
    # behavior.  simple_fsdp has no equivalent of FSDP2's per-boundary casting,
    # so we cast the buffer explicitly after it is recomputed in init_weights.
    model._mp_param_dtype = TORCH_DTYPE_MAP[training.mixed_precision_param]
