...
def create_vllm_engine(config: RLTrainer.Config) -> LLMEngine:
    ...
    if gen_config.batch_invariant_mode:
        from torchtitan.experiments.rl.batch_invariant import (
            enable_batch_invariant_mode,
        )

        enable_batch_invariant_mode()
        # Enable vLLM-specific batch-invariant attention path.
...
def build_trainer_model(config):
    ...
    # Apply batch-invariant settings before model build
    if config.trainer.batch_invariant_mode:
        from torchtitan.experiments.rl.batch_invariant import (
            enable_batch_invariant_mode,
        )
    ...
    # Activate FA3 for bitwise-identical logprobs with the generator
    if config.trainer.batch_invariant_mode and has_cuda_capability(9, 0):
...
def _test_config() -> RLTrainer.Config:
    """Test-specific config: stochastic sampling, fewer tokens, single sample."""
