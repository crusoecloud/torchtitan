...
def build_policy_gradient_loss(
    ...
    print(
        f"build_policy_gradient_loss: enable={compile_config.enable}, "
        f"components={compile_config.components}"
    )
    if False:
        # if compile_config.enable and "loss" in compile_config.components:
        print("Compiling the RL policy gradient loss with torch.compile (inductor)")
        loss_fn = torch.compile(loss_fn, backend="inductor")
    return loss_fn
