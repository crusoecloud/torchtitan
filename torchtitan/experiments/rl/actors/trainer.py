...
class PolicyTrainer(Actor, Configurable):
    ...
    def __init__(
        ...
        *,
        ...
        # Give ref model blocks a distinct forward code object so dynamo caches
        # them independently from the policy model (avoids AsyncCollectiveTensor
        # guard recompile from grad mode differences).
        ...
        # Constant upper bound for VarlenMetadata max_q/max_k, derived from
        # the RoPE cache length.  Passing this to compute_token_log_probs
        # prevents dynamo from specializing on per-episode sequence lengths.
        self.max_seq_len: int = model.freqs_cis.shape[0]
